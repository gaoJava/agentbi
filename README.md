...
AgentBI 通过 Superset API 保存图表和执行真实 Dataset 查询，页面不直接暴露 Superset UI。
当前中文建图向导与原生画布支持：表格、指标卡、柱状图、折线图、面积图、饼图、
环图、散点图和漏斗图。地图、热力图、雷达图、箱线图及需要多个维度或多个指标的
复杂图表暂不开放；从 Superset 同步到这些类型时，AgentBI 会明确提示“尚未适配”，
不会用错误图形替代。
