...
管理员可以在“用户与角色”页面新增账号，或修改已有账号的角色、数据范围和
启停状态。系统不物理删除用户，因为报告、会话和审计事件都需要保留创建人证据；
业务上的“删除账号”对应停用，停用后已有会话会在下一次请求时立即失效。

`user`、`admin` 是系统内置角色，不能修改或删除。管理员可以新增自定义角色并
从原子权限清单中勾选最小权限集合；自定义角色代码创建后不可修改。已经分配给
用户的角色不能删除，必须先迁移相关用户，避免产生孤立授权关系。
治理接口按 `dashboard:manage`、`semantic_model:manage`、`datasource:manage`、
`user:manage`、`audit:view` 分别校验，不依赖角色名称；因此自定义角色勾选的
权限会在后端和菜单上同步生效。
